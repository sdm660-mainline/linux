...
static const struct regulator_bulk_data nt36672_txd_supplies[] = {
	{ .supply = "vddio" },
	{ .supply = "lab" },
	{ .supply = "ibb" },
};
...
static int nt36672_txd_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(nt36672_txd_supplies), ctx->supplies);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		gpiod_set_value_cansleep(ctx->reset_gpio, 1);
		regulator_bulk_disable(ARRAY_SIZE(nt36672_txd_supplies), ctx->supplies);
		return ret;
	}
...
static int nt36672_txd_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx = devm_drm_panel_alloc(dev, struct nt36672_txd, panel,
				   &nt36672_txd_panel_funcs,
				   DRM_MODE_CONNECTOR_DSI);
	if (IS_ERR(ctx))
		return PTR_ERR(ctx);

	ret = devm_regulator_bulk_get_const(dev,
					    ARRAY_SIZE(nt36672_txd_supplies),
					    nt36672_txd_supplies,
					    &ctx->supplies);
	if (ret < 0)
		return ret;
