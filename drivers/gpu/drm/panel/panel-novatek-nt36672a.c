...
static int tianma_lavender_init_cmds(struct drm_panel *panel)
{
	struct nt36672a_panel *pinfo = to_nt36672a_panel(panel);
	struct mipi_dsi_multi_context dsi_ctx = { .dsi = pinfo->link };
	...
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xff, 0x26);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xfb, 0x01);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, MIPI_DCS_READ_PPS_CONTINUE, 0x12);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xaa, 0x10);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xae, 0x8a);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xff, 0x10);

	// mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0x11, 0x00); // MIPI_DCS_EXIT_SLEEP_MODE
	mipi_dsi_dcs_exit_sleep_mode_multi(&dsi_ctx);
	...
	// mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0x29, 0x00); // MIPI_DCS_SET_DISPLAY_ON
	mipi_dsi_dcs_set_display_on_multi(&dsi_ctx);

	return dsi_ctx.accum_err;
}

static int tianma_lavender_off_cmds(struct drm_panel *panel)
{
	struct nt36672a_panel *pinfo = to_nt36672a_panel(panel);
	struct mipi_dsi_multi_context dsi_ctx = { .dsi = pinfo->link };
...
static const struct nt36672a_panel_desc tianma_lavender_panel_desc = {
	.display_mode = &tianma_lavender_panel_default_mode,
...
static const struct of_device_id tianma_fhd_video_of_match[] = {
	...
	{ .compatible = "tianma,nt36672a-xiaomi-lavender", .data = &tianma_lavender_panel_desc },
	{ },
};
