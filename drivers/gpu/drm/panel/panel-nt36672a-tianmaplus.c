...
static const struct regulator_bulk_data nt36672a_tianmaplus_supplies[] = {
	{ .supply = "vddio" },
	{ .supply = "vsn" },
	{ .supply = "vsp" },
};
...
static int nt36672a_tianmaplus_off(struct nt36672a_tianmaplus *ctx)
{
	...
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0x28, 0x00); // MIPI_DCS_SET_DISPLAY_OFF
	mipi_dsi_msleep(&dsi_ctx, 20);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0x10, 0x00); // MIPI_DCS_ENTER_SLEEP_MODE
...
static int nt36672a_tianmaplus_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(nt36672a_tianmaplus_supplies), ctx->supplies);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		gpiod_set_value_cansleep(ctx->reset_gpio, 1);
		regulator_bulk_disable(ARRAY_SIZE(nt36672a_tianmaplus_supplies), ctx->supplies);
		return ret;
	}
...
{
	return drm_connector_helper_get_modes_fixed(connector, &nt36672a_tianmaplus_mode);
}
...
static int nt36672a_tianmaplus_probe(struct mipi_dsi_device *dsi)
{
	...
	ret = devm_regulator_bulk_get_const(dev,
					    ARRAY_SIZE(nt36672a_tianmaplus_supplies),
					    nt36672a_tianmaplus_supplies,
					    &ctx->supplies);
	if (ret < 0)
		return ret;
