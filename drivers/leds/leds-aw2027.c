...
static const unsigned char aw2027_reg_access[AW2027_REG_MAX] = {
				...
				struct device_node *node)
{
	...
	for_each_child_of_node(node, temp) {
		...
		rc = of_property_read_u32(temp, "aw2027,id", &tmp);
		if (rc < 0) {
			dev_err(&led->client->dev,
		...
		rc = of_property_read_u32(temp, "aw2027,imax", &tmp);
		if (rc < 0) {
			dev_err(&led->client->dev,
		...
		rc = of_property_read_u32(temp, "aw2027,led-current", &tmp);
		if (rc < 0) {
			dev_err(&led->client->dev,
				"Failure reading led-current, rc = %d\n", rc);
			goto free_pdata;
		}
		led->pdata->led_current = tmp;

		rc = of_property_read_u32(temp, "aw2027,max-brightness", &tmp);
		if (rc < 0) {
			dev_err(&led->client->dev,
		...
		rc = of_property_read_u32(temp, "aw2027,rise-time-ms", &tmp);
		if (rc < 0) {
			dev_err(&led->client->dev,
				"Failure reading rise-time-ms, rc = %d\n", rc);
			goto free_pdata;
		}
		led->pdata->rise_time_ms = tmp;

		rc = of_property_read_u32(temp, "aw2027,hold-time-ms", &tmp);
		if (rc < 0) {
			dev_err(&led->client->dev,
				"Failure reading hold-time-ms, rc = %d\n", rc);
			goto free_pdata;
		}
		led->pdata->hold_time_ms = tmp;

		rc = of_property_read_u32(temp, "aw2027,fall-time-ms", &tmp);
		if (rc < 0) {
			dev_err(&led->client->dev,
				"Failure reading fall-time-ms, rc = %d\n", rc);
			goto free_pdata;
		}
		led->pdata->fall_time_ms = tmp;

		rc = of_property_read_u32(temp, "aw2027,off-time-ms", &tmp);
		if (rc < 0) {
			dev_err(&led->client->dev,
				"Failure reading off-time-ms, rc = %d\n", rc);
			goto free_pdata;
		}
		led->pdata->off_time_ms = tmp;
...
static void aw2027_led_remove(struct i2c_client *client)
{
...
static const struct of_device_id aw2027_match_table[] = {
	{ .compatible = "awinic,aw2027_led", },
	{ },
};
MODULE_DEVICE_TABLE(of, aw2027_match_table);
...
module_i2c_driver(aw2027_led_driver);
